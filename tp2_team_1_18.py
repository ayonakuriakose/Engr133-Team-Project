...
def gaussian_filter(gray_array,sigma):
    #Finds kernel size using given formula and makes sure the kernel is odd
    ...
    for x in range(gray_array.shape[0]):
        ...
        for y in range(gray_array.shape[1]):
            #finds the region surounding the center pixel
    ...
    return np.clip(blurred, 0, 254.999).astype(np.uint8)
